...
task :setup do
  ...
  ['merchant', 'merchant_test'].each do |database|
    ...
    connection.exec("CREATE TABLE offers(id SERIAL PRIMARY KEY, title VARCHAR(60), description VARCHAR(240), price NUMERIC, currency VARCHAR(10), created_at VARCHAR(20), duration VARCHAR(10), status VARCHAR(20));")
  end
end
